...
export const fetchProductCategories = async (): Promise<ProductCategory[]> => {
    let response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/computer-component-categories`);
...
export const fetchProductBrands = async (): Promise<ProductBrand[]> => {
    return [];
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/product-brands`);
