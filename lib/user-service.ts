...
export const fetchUser = async (requestRole: string, token: string): Promise<GetUserResponseAPI> => {
    ...
    const response = await fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/user` + queryString, {
        headers: {
...
export const fetchUserDefault = async (): Promise<GetUserResponseAPI> => {
    const response = await fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/user/show-default`, {
        headers: {
            "Content-Type": "application/json"
        }
    });
