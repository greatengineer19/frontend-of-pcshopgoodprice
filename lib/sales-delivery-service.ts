...
export const voidSalesDelivery = async (id: number): Promise<any> => {  
    try {
        ...
        const response = await fetch(
            `http://${process.env.NEXT_PUBLIC_API_URL}/api/sales-deliveries/${id}/void`,
            {
                method: 'PATCH',
                headers: {
...
export const fullyDelivered = async (id: number): Promise<any> => {  
    try {
        ...
        const response = await fetch(
            `http://${process.env.NEXT_PUBLIC_API_URL}/api/sales-deliveries/${id}/fully_delivered`,
            {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
...
export const fetchSalesDeliveries = async (): Promise<SalesDelivery[]> => {
    ...
    const response = await fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/sales-deliveries`, {
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
        }
